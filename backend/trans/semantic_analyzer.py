...
class SemanticAnalyzer:
        ...
        if self.current_scope and self.current_scope.parent:
            ...
            for symbol in self.current_scope.symbols.values():
                if symbol.type == 'variable' and not symbol.used:
                    self._warning(f"Переменная '{symbol.name}' объявлена, но не используется", symbol.lineno)
            
            self.current_scope = self.current_scope.parent
    
    def _report_diagnostics(self):
        """Выводит результаты семантического анализа"""
        if self.errors or self.warnings:
            ...
            if self.errors:
                print(f"\nОбнаружено {len(self.errors)} ошибок, {len(self.warnings)} предупреждений")
            else:
                print(f"\nОшибок не обнаружено, {len(self.warnings)} предупреждений")
        else:
            print("✓ Семантический анализ: ошибок не обнаружено")
    ...
    def _visit_variable(self, node):
        ...
        if not symbol:
            ...
            self._warning(f"Использование неинициализированной переменной '{node.name}'")
        else:
